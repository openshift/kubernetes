...
func Test_updateEndpointSliceLabels(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "b contains a",
			a:    map[string]string{"foo": "bar"},
			b:    map[string]string{"foo": "bar", "foo2": "bar2"},
			want: false,
		},
		{
			name: "a contains b",
			a:    map[string]string{"foo": "bar", "foo2": "bar2"},
			b:    map[string]string{"foo": "bar"},
			want: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := updateEndpointSliceLabels(tt.a, tt.b); got != tt.want {
				t.Errorf("updateEndpointSliceLabels() = %v, want %v", got, tt.want)
			}
		})
	}
}

func Test_updateEndpointSliceAnnotations(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "a empty b with annotations",
			b:    map[string]string{"foo": "bar"},
			want: true,
		},
		{
			name: "a with annotations b empty",
			a:    map[string]string{"foo": "bar"},
			want: true,
		},
		{
			name: "a and b same annotations",
			a:    map[string]string{"foo": "bar"},
			b:    map[string]string{"foo": "bar"},
			want: false,
		},
		...
		{
			name: "a and b same annotations, a with triggertime",
			a: map[string]string{
				"foo":                                 "bar",
				corev1.EndpointsLastChangeTriggerTime: "date",
			},
		...
		{
			name: "a and b same annotations, b with triggertime",
			a:    map[string]string{"foo": "bar"},
			b: map[string]string{
				"foo":                                 "bar",
				corev1.EndpointsLastChangeTriggerTime: "date",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := updateEndpointSliceAnnotations(tt.a, tt.b); got != tt.want {
				t.Errorf("updateEndpointSliceAnnotations() = %v, want %v", got, tt.want)
			}
		})
	}
}
