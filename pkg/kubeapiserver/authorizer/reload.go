...
func (r *reloadableAuthorizerResolver) newForConfig(authzConfig *authzconfig.AuthorizationConfiguration) (authorizer.Authorizer, authorizer.RuleResolver, error) {
	...
	authorizers = append(authorizers, minimumkubeletversion.NewMinimumKubeletVersion(nodeidentifier.NewDefaultNodeIdentifier(), r.initialConfig.VersionedInformerFactory.Core().V1().Nodes().Lister()))
