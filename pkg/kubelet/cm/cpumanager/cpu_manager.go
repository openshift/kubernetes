...
func (m *manager) Allocate(p *v1.Pod, c *v1.Container) error {
	...
	logKey := "alloc:" + klog.KObj(p).String()
	m.removeStaleState(logKey)
...
func (m *manager) GetTopologyHints(pod *v1.Pod, container *v1.Container) map[string][]topologymanager.TopologyHint {
	...
	logKey := "getHints:" + klog.KObj(pod).String()
	m.removeStaleState(logKey)
...
func (m *manager) GetPodTopologyHints(pod *v1.Pod) map[string][]topologymanager.TopologyHint {
	...
	m.setPodPendingAdmission(pod)
	// Garbage collect any stranded resources before providing TopologyHints
	logKey := "getPodHints:" + klog.KObj(pod).String()
	m.removeStaleState(logKey)
	// Delegate to active policy
...
func (m *manager) removeStaleState(logKey interface{}) {
	...
	klog.V(4).InfoS("RemoveStaleState: begin", "logKey", logKey, "activeContainers", activeContainersCount)
	...
	for podUID := range assignments {
		for containerName := range assignments[podUID] {
			...
			if _, ok := activeContainers[podUID][containerName]; ok {
				klog.V(5).InfoS("RemoveStaleState: container still active", "logKey", logKey, "podUID", podUID, "containerName", containerName)
				continue
			}
			klog.V(2).InfoS("RemoveStaleState: removing container", "logKey", logKey, "podUID", podUID, "containerName", containerName)
			err := m.policyRemoveContainerByRef(podUID, containerName)
			if err != nil {
				klog.ErrorS(err, "RemoveStaleState: failed to remove container", "logKey", logKey, "podUID", podUID, "containerName", containerName)
			}
			assignmentRemoved++
		}
	}
	...
	m.containerMap.Visit(func(podUID, containerName, containerID string) {
		if _, ok := activeContainers[podUID][containerName]; ok {
			klog.V(5).InfoS("RemoveStaleState: containerMap: container still active", "logKey", logKey, "podUID", podUID, "containerName", containerName)
			return
		}
	...
	klog.V(4).InfoS("RemoveStaleState: done", "logKey", logKey, "activeContainers", activeContainersCount, "removed", assignmentRemoved, "mapRemoval", containerMapRemoved)
}
...
func (m *manager) reconcileState() (success []reconciledContainer, failure []reconciledContainer) {
	...
	logKey := fmt.Sprintf("reconcile%v", time.Now().Unix())
	klog.V(4).InfoS("ReconcileState: start", "logKey", logKey)
	...
	for _, pod := range m.activePods() {
		...
		if !ok {
			klog.V(5).InfoS("ReconcileState: skipping pod; status not found", "logKey", logKey, "pod", klog.KObj(pod))
		...
		for _, container := range allContainers {
			...
			if err != nil {
				klog.V(5).InfoS("ReconcileState: skipping container; ID not found in pod status", "logKey", logKey, "pod", klog.KObj(pod), "containerName", container.Name, "err", err)
			...
			if err != nil {
				klog.V(5).InfoS("ReconcileState: skipping container; container status not found in pod status", "logKey", logKey, "pod", klog.KObj(pod), "containerName", container.Name, "err", err)
				failure = append(failure, reconciledContainer{pod.Name, container.Name, ""})
				continue
			}
			...
			if cstatus.State.Waiting != nil ||
				...
				klog.V(4).InfoS("ReconcileState: skipping container; container still in the waiting state", "logKey", logKey, "pod", klog.KObj(pod), "containerName", container.Name, "err", err)
				failure = append(failure, reconciledContainer{pod.Name, container.Name, ""})
				continue
			}
			...
			if cstatus.State.Terminated != nil {
				...
				if err == nil {
					klog.V(4).InfoS("ReconcileState: ignoring terminated container", "logKey", logKey, "pod", klog.KObj(pod), "containerID", containerID)
				}
				m.Unlock()
				continue
			}
			...
			if !cset.Equals(lcset) {
				klog.V(4).InfoS("ReconcileState: updating container", "logKey", logKey, "pod", klog.KObj(pod), "containerName", container.Name, "containerID", containerID, "cpuSet", cset)
				err = m.updateContainerCPUSet(ctx, containerID, cset)
				if err != nil {
					klog.ErrorS(err, "ReconcileState: failed to update container", "logKey", logKey, "pod", klog.KObj(pod), "containerName", container.Name, "containerID", containerID, "cpuSet", cset)
					failure = append(failure, reconciledContainer{pod.Name, container.Name, containerID})
					continue
				}
				m.lastUpdateState.SetCPUSet(string(pod.UID), container.Name, cset)
			} else {
				klog.V(5).InfoS("ReconcileState: container up to date", "logKey", logKey, "pod", klog.KObj(pod), "containerName", container.Name, "containerID", containerID, "cpuSet", cset)
			}
	...
	klog.V(4).InfoS("ReconcileState: done", "logKey", logKey, "successCount", len(success), "failureCount", len(failure))
