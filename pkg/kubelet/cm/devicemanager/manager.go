...
func (m *ManagerImpl) logStateUnlocked(keysAndValues ...any) {
	klog.V(2).InfoS("DBG alloctd devices", append(keysAndValues, "allocated", stringifyDeviceMap(m.allocatedDevices))...)
	klog.V(2).InfoS("DBG healthy devices", append(keysAndValues, "healthy", stringifyDeviceMap(m.allocatedDevices))...)
}
