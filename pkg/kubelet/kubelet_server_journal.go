...
func (n *nodeLogQuery) validate() field.ErrorList {
	...
	if !allowedFormats.Has(n.Format) {
		allErrs = append(allErrs, field.Invalid(field.NewPath("format"), n.Format,
			fmt.Sprintf("only the following values are allowed: %v", allowedFormats)))
	}
