	...
	for _, test := range []struct {
		...
		t.Run(test.description, func(t *testing.T) {
			...
			for _, remain := range test.remain {
				status, err := fakeRuntime.PodSandboxStatus(context.Background(), fakeSandboxes[remain].Id)
				assert.NoError(t, err)
...
func TestContainerGC(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.description, func(t *testing.T) {
			...
			assert.Len(t, realRemain, len(test.remain))
			for _, remain := range test.remain {
				status, err := fakeRuntime.ContainerStatus(context.Background(), fakeContainers[remain].Id)
				assert.NoError(t, err)
