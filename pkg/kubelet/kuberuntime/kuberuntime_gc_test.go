...
func TestSandboxGC(t *testing.T) {
	...
	makeGCSandbox := func(pod *v1.Pod, attempt uint32, state internalapi.PodSandboxState, withPodStateProvider bool, createdAt int64) sandboxTemplate {
	...
	for c, test := range []struct {
	...
	}{
		{
			...
			sandboxes: []sandboxTemplate{
				makeGCSandbox(pods[2], 0, internalapi.PodSandboxState_SANDBOX_NOTREADY, false, 0),
			},
		...
		},
		{
			...
			sandboxes: []sandboxTemplate{
				makeGCSandbox(pods[2], 0, internalapi.PodSandboxState_SANDBOX_READY, false, 0),
			},
			containers:          []containerTemplate{},
			remain:              []int{0},
			evictTerminatedPods: false,
		},
		{
			...
			sandboxes: []sandboxTemplate{
				makeGCSandbox(pods[0], 0, internalapi.PodSandboxState_SANDBOX_READY, true, 0),
				makeGCSandbox(pods[1], 0, internalapi.PodSandboxState_SANDBOX_NOTREADY, true, 0),
			},
			containers:          []containerTemplate{},
			remain:              []int{0, 1},
			evictTerminatedPods: false,
		},
		{
			...
			sandboxes: []sandboxTemplate{
				makeGCSandbox(pods[0], 1, internalapi.PodSandboxState_SANDBOX_NOTREADY, true, 1),
				makeGCSandbox(pods[0], 0, internalapi.PodSandboxState_SANDBOX_NOTREADY, true, 0),
			},
			containers:          []containerTemplate{},
			remain:              []int{0},
			evictTerminatedPods: false,
		...
		},
		{
			...
			sandboxes: []sandboxTemplate{
				makeGCSandbox(pods[0], 0, internalapi.PodSandboxState_SANDBOX_NOTREADY, false, 0),
			},
			containers: []containerTemplate{
		...
		},
		{
			...
			sandboxes: []sandboxTemplate{
				// running sandbox.
				makeGCSandbox(pods[0], 1, internalapi.PodSandboxState_SANDBOX_READY, true, 1),
				// exited sandbox without containers.
				makeGCSandbox(pods[0], 0, internalapi.PodSandboxState_SANDBOX_NOTREADY, true, 0),
				// exited sandbox with containers.
				makeGCSandbox(pods[1], 1, internalapi.PodSandboxState_SANDBOX_NOTREADY, true, 1),
				// exited sandbox without containers.
				makeGCSandbox(pods[1], 0, internalapi.PodSandboxState_SANDBOX_NOTREADY, true, 0),
...
func makeGCContainer(podStateProvider *fakePodStateProvider, podName, containerName string, attempt int, createdAt int64, state internalapi.ContainerState) containerTemplate {
...
func TestContainerGC(t *testing.T) {
	...
	for c, test := range []struct {
	...
	}{
		{
			...
			containers: []containerTemplate{
				makeGCContainer(podStateProvider, "foo", "bar", 0, 0, internalapi.ContainerState_CONTAINER_EXITED),
			},
		...
		},
		{
			...
			containers: []containerTemplate{
				makeGCContainer(podStateProvider, "foo", "bar", 4, 4, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo", "bar", 3, 3, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo", "bar", 2, 2, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo", "bar", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo", "bar", 0, 0, internalapi.ContainerState_CONTAINER_EXITED),
			},
		...
		},
		{
			...
			containers: []containerTemplate{
				makeGCContainer(podStateProvider, "foo", "bar", 2, time.Now().UnixNano(), internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo", "bar", 1, time.Now().UnixNano(), internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo", "bar", 0, time.Now().UnixNano(), internalapi.ContainerState_CONTAINER_EXITED),
			},
			remain:              []int{0, 1, 2},
			evictTerminatedPods: false,
			allSourcesReady:     true,
		...
		},
		{
			...
			containers: []containerTemplate{
				makeGCContainer(podStateProvider, "foo", "bar", 2, 2, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo", "bar", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo", "bar", 0, 0, internalapi.ContainerState_CONTAINER_RUNNING),
			},
			remain:              []int{0, 1, 2},
			evictTerminatedPods: false,
			allSourcesReady:     true,
		...
		},
		{
			...
			containers: []containerTemplate{
				...
				makeGCContainer(podStateProvider, "foo", "bar", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo", "bar", 0, 0, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo1", "baz", 2, 2, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo1", "baz", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo1", "baz", 0, 0, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo2", "bar", 2, 2, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo2", "bar", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo2", "bar", 0, 0, internalapi.ContainerState_CONTAINER_EXITED),
			},
		...
		},
		{
			...
			containers: []containerTemplate{
				makeGCContainer(podStateProvider, "foo", "bar", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo", "bar", 0, 0, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo1", "bar1", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo1", "bar1", 0, 0, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo2", "bar2", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo2", "bar2", 0, 0, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo3", "bar3", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo3", "bar3", 0, 0, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo4", "bar4", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo4", "bar4", 0, 0, internalapi.ContainerState_CONTAINER_EXITED),
			},
		...
		},
		{
			...
			containers: []containerTemplate{
				makeGCContainer(podStateProvider, "foo", "bar", 2, 2, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo", "bar", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo1", "bar1", 2, 2, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo1", "bar1", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo2", "bar2", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo3", "bar3", 0, 0, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo4", "bar4", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo5", "bar5", 0, 0, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo6", "bar6", 2, 2, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo7", "bar7", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
			},
		...
		},
		{
			...
			containers: []containerTemplate{
				...
				makeGCContainer(podStateProvider, "foo1", "bar1", 2, 2, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo1", "bar1", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "running", "bar2", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "foo3", "bar3", 0, 0, internalapi.ContainerState_CONTAINER_RUNNING),
			},
			remain:              []int{4, 5},
			evictTerminatedPods: true,
			allSourcesReady:     true,
		},
		{
			...
			containers: []containerTemplate{
				...
				makeGCContainer(podStateProvider, "deleted", "bar1", 2, time.Now().UnixNano(), internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "deleted", "bar1", 1, 1, internalapi.ContainerState_CONTAINER_EXITED),
				makeGCContainer(podStateProvider, "deleted", "bar1", 0, 0, internalapi.ContainerState_CONTAINER_EXITED),
			},
			remain:              []int{0, 1, 2},
			evictTerminatedPods: false,
			allSourcesReady:     true,
...
func TestUnknownStateContainerGC(t *testing.T) {
	...
	fakeContainers := makeFakeContainers(t, m, []containerTemplate{
		makeGCContainer(podStateProvider, "foo", "bar", 0, 0, internalapi.ContainerState_CONTAINER_UNKNOWN),
	})
	fakeRuntime.SetFakeContainers(fakeContainers)
