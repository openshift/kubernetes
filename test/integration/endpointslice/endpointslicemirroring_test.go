...
func TestEndpointSliceMirroringUpdates(t *testing.T) {
	...
	for i, tc := range testCases {
		t.Run(tc.testName, func(t *testing.T) {
			...
			// Obtain ResourceVersion of the new endpoint slice created
			...
			if err := wait.PollImmediate(1*time.Second, wait.ForeverTestTimeout, func() (bool, error) {
				...
				endpointSlice := esList.Items[0]
				resVersion = endpointSlice.ObjectMeta.ResourceVersion
				return true, nil
			}); err != nil {
				t.Fatalf("endpoint slice not found: %v", err)
			}
			...
			err = wait.PollImmediate(1*time.Second, wait.ForeverTestTimeout, func() (bool, error) {
				...
				endpointSlice := esList.Items[0]
				if endpointSlice.ObjectMeta.ResourceVersion == resVersion {
					t.Logf("Waiting for EndpointSlice to be updated")
					return false, nil
				}
				...
				if !assert.ElementsMatch(t, epAddresses, sliceAddresses) {
					return false, fmt.Errorf("Expected EndpointSlice to have the same IP addresses, expected %v got %v", epAddresses, sliceAddresses)
				}

				// check labels were mirrored
				if !isSubset(customEndpoints.Labels, endpointSlice.Labels) {
					return false, fmt.Errorf("Expected EndpointSlice to mirror labels, expected %v to be in received %v", customEndpoints.Labels, endpointSlice.Labels)
				}
				...
				if !apiequality.Semantic.DeepEqual(annotations, endpointSlice.Annotations) {
					return false, fmt.Errorf("Expected EndpointSlice to mirror annotations, expected %v received %v", customEndpoints.Annotations, endpointSlice.Annotations)
				}
				return true, nil
			})
			if err != nil {
