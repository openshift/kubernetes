...
`, server.ClientConfig.Host, server.ServerOpts.SecureServing.ServerCert.CertKey.CertFile))
	...
	testsWithoutInsecureServing := []struct {
		name       string
		tester     componentTester
		extraFlags []string
	}{
	...
	for _, tt := range testsWithoutInsecureServing {
		t.Run(tt.name, func(t *testing.T) {
			testComponentWithoutInsecureServing(t, tt.tester, apiserverConfig.Name(), brokenApiserverConfig.Name(), token, tt.extraFlags)
		})
	}
}
...
func testComponentWithoutInsecureServing(t *testing.T, tester componentTester, kubeconfig, brokenKubeconfig, token string, extraFlags []string) {
	tests := []struct {
		name           string
		flags          []string
		path           string
