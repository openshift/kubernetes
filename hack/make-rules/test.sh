...
produceJUnitXMLReport() {
  ...
  if ! command -v gotestsum >/dev/null 2>&1; then
    ...
    pushd "${KUBE_ROOT}/hack/tools" >/dev/null
      # CI may set GOFLAGS=-mod=vendor; strict vendor checks can fail on staging
      # requires even when the main test run already used -mod=mod.
      GO111MODULE=on GOFLAGS="-mod=readonly" go install gotest.tools/gotestsum
    popd >/dev/null
  fi
