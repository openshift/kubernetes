...
func TestAuthorize(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedMsg:     `kubelet version of node node0 is 1.29.8, which is lower than minimumKubeletVersion of 1.30.0`,
			node: &v1.Node{ObjectMeta: metav1.ObjectMeta{Name: "node0"},
				Status: v1.NodeStatus{
					NodeInfo: v1.NodeSystemInfo{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			nodeStore.Add(tc.node)

			authorizer := NewMinimumKubeletVersion(identifier, listersv1.NewNodeLister(nodeStore))
