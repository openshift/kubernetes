...
func TestEmailVerifiedUsageMatchesUpstream(t *testing.T) {
	// This test ensures that our local AST walker functions (usesEmailClaim,
	// usesEmailVerifiedClaim, hasSelectExp, etc.) stay in sync with the upstream
	// implementations they were copied from:
	...
	//
	// If this test fails after a rebase, it likely means the upstream functions
	// have changed and our local copies need to be updated accordingly.
	testcases := []struct {
		name                    string
		expression              string
		expectUsesEmail         bool
		expectUsesEmailVerified bool
	}{
		{
			name:                    "claims.email",
			expression:              "claims.email",
			expectUsesEmail:         true,
			expectUsesEmailVerified: false,
		},
		{
			name:                    "claims.email_verified",
			expression:              "claims.email_verified",
			expectUsesEmail:         false,
			expectUsesEmailVerified: true,
		},
		{
			name:                    "claims.email_verified ? claims.email : unverified",
			expression:              "claims.email_verified ? claims.email : 'unverified'",
			expectUsesEmail:         true,
			expectUsesEmailVerified: true,
		},
		...
		{
			name:                    "has(claims.email) ? claims.email : claims.sub",
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			accessor := &authenticationcel.ClaimMappingExpression{Expression: tc.expression}
			result, errs := validateClaimMappingCELExpression(context.TODO(), cel, &costRecorder{}, field.NewPath("test"), accessor)
			if len(errs) > 0 {
				t.Fatalf("unexpected compilation error: %v", errs)
			}
			if result == nil || result.AST == nil {
				t.Fatal("expected non-nil compilation result with AST")
			}

			if got := usesEmailClaim(result.AST); got != tc.expectUsesEmail {
				t.Errorf("usesEmailClaim: expected %v but got %v", tc.expectUsesEmail, got)
			}

			if got := usesEmailVerifiedClaim(result.AST); got != tc.expectUsesEmailVerified {
				t.Errorf("usesEmailVerifiedClaim: expected %v but got %v", tc.expectUsesEmailVerified, got)
			}
		})
	}
}
