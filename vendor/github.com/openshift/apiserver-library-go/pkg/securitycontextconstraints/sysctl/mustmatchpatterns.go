...
func getSafeSysctlAllowlist(getVersion func() (*version.Version, error)) []string {
	...
	if err != nil {
		klog.Error(err, "failed to get kernel version, unable to determine which sysctls are available")
	}
	...
	for _, sc := range safeSysctls {
		...
		if kernelVersion == nil {
			klog.Info("kernel version unknown, skipping sysctl", "sysctl", sc.name)
			continue
		}
