...
func (c *constraint) computeSecurityContext(
	...
	constraints, err := c.sccs(requiredSCCName, validatedSCCHint, specMutationAllowed)
	if err != nil {
		return nil, "", nil, admission.NewForbidden(a, err)
	}

	providers, err := c.providers(ctx, a, pod, constraints)
	if err != nil {
		return nil, "", nil, err
	}
...
loop:
	...
	if validatedSCCSubjectType := sccChecker.allowedFor(ctx, allowingProvider); validatedSCCSubjectType != "" {
		if allowedPod.Annotations == nil {
			allowedPod.Annotations = make(map[string]string)
		}

		allowedPod.Annotations[securityv1.ValidatedSCCSubjectTypeAnnotation] = validatedSCCSubjectType
	}
