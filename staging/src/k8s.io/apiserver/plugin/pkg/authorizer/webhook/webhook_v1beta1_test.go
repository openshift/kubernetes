...
`,
	...
	for _, tt := range tests {
		...
		err := func() error {
			...
			sarClient, err := subjectAccessReviewInterfaceFromKubeconfig(p, "v1beta1", DefaultRetryBackoff, nil)
			if err != nil {
				return fmt.Errorf("error building sar client: %v", err)
			}
			_, err = newWithBackoff(sarClient, 0, 0, testRetryBackoffWithNoDelay)
			return err
		}()
...
func newV1beta1Authorizer(callbackURL string, clientCert, clientKey, ca []byte, cacheTime time.Duration) (*WebhookAuthorizer, error) {
	...
	sarClient, err := subjectAccessReviewInterfaceFromKubeconfig(p, "v1beta1", testRetryBackoffWithNoDelay, nil)
	if err != nil {
		return nil, fmt.Errorf("error building sar client: %v", err)
	}
	return newWithBackoff(sarClient, cacheTime, cacheTime, testRetryBackoffWithNoDelay)
}
