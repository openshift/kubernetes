...
func GetServicePrincipalToken(config *AzureAuthConfig, env *azure.Environment) (azcore.TokenCredential, error) {
	...
	if len(config.AADClientSecret) > 0 {
		...
		return azidentity.NewClientSecretCredential(tenantID, config.AADClientID, config.AADClientSecret, &azidentity.ClientSecretCredentialOptions{ClientOptions: clientOptions})
	}
	...
	if len(config.AADClientCertPath) > 0 && len(config.AADClientCertPassword) > 0 {
		...
		certificate, privateKey, err := azidentity.ParseCertificates(certData, nil)
		if err != nil {
			return nil, fmt.Errorf("decoding the client certificate: %v", err)
		}

		return azidentity.NewClientCertificateCredential(tenantID, config.AADClientID, certificate, privateKey, &azidentity.ClientCertificateCredentialOptions{ClientOptions: clientOptions})
	}
